...
describe('YouTubeDataProvider', () => {
  it('should return transcription from youtube video', async () => {
    ...
    expect(documents[0].content.toLowerCase()).toContain("all right, so here we are, in front of the")
    expect(documents[0].content.toLowerCase()).toContain("elephants")
    expect(documents[0].content.toLowerCase()).toContain("the cool thing about these guys is that they")
    expect(documents[0].content.toLowerCase()).toContain("have really...")
    expect(documents[0].content.toLowerCase()).toContain("really really long trunks")
    ...
    expect(documents[0].content.toLowerCase()).toContain("and that's pretty much all there is to")
    expect(documents[0].content.toLowerCase()).toContain("say")
  });

  it('should return transcription from multiple youtube videos from a specific channel', async () => {
    ...
    expect(documents).toContainEqual({
      content: expect.stringContaining("hi everybody my name is eric and i'm one"),
      metadata: {
    ...
    expect(documents).toContainEqual({
      content: expect.stringContaining("stop watching youtube tutorials to learn"),
      metadata: {
