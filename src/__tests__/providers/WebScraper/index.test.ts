...
test("WebScraper Crawl model", async () => {
...
test("WebScraper Single Urls mode", async () => {
  const webDataConnector = createDataConnector({
    provider: "web-scraper",
  });

  await webDataConnector.setOptions({
    urls: ["https://docs.mendable.ai/applications/routers", "https://docs.mendable.ai/integrations/slack"],
    mode: "single_urls",
  });
